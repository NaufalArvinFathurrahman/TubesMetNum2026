...
void recalculateTraffic(bool isJamSibuk, double& intervalNS, double& intervalEW, double& siklusVisual, double& siklusMatematis) {
    ...
    MathEngine::lu_decomposition(Matriks_A, L, U);
    std::vector<double> flow_mentah = MathEngine::solve_lu(L, U, Vektor_B);
    ...
    siklusMatematis = MathEngine::newton_raphson(60.0, param_a, param_b);
    ...
    if (isJamSibuk) {
        ...
        intervalNS = (60.0 / ((flow_mentah[0] + flow_mentah[2]) / 2)) * 0.5;
        intervalEW = (60.0 / ((flow_mentah[1] + flow_mentah[3]) / 2)) * 0.5;
    } else {
        // Pada jam sepi, mobil jarang muncul (interval besar)
        intervalNS = (60.0 / ((flow_mentah[0] + flow_mentah[2]) / 2)) * 4.0;
        intervalEW = (60.0 / ((flow_mentah[1] + flow_mentah[3]) / 2)) * 4.0;
    }
...
int main() {
    ...
    bool isJamSibuk = false; // Memulai simulasi pada jam 06:00 pagi (Jam Sepi)
    double interval_ns, interval_ew, siklus_visual, siklus_matematis;
    
    // Lakukan perhitungan pertama kalinya
    recalculateTraffic(isJamSibuk, interval_ns, interval_ew, siklus_visual, siklus_matematis);
    ...
    Intersection dunia(siklus_visual);
    ...
    while (!WindowShouldClose()) {
        ...
        dunia.updateWorld(dt, interval_ns, interval_ew);
        ...
        DrawText(isJamSibuk ? "Status: JAM SIBUK" : "Status: JAM SEPI", 10, 40, 20, isJamSibuk ? RED : BLUE);
