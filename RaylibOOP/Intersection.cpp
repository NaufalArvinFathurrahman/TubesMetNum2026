// ==========================================
// IMPLEMENTASI PENGATUR PERSIMPANGAN (Intersection.cpp)
// ==========================================
...
Intersection::Intersection(double calculatedCycleTime) {
    optimalCycleTime = calculatedCycleTime;
    timerLampu = 0.0f;
    timerSpawn = 0.0f;
    currentPhase = 0; // Mulai dengan arah Utara (N) = Hijau
    isYellowPhase = false;
    yellowDuration = 3.0; // Ini batas default awal, aslinya diatur dinamis di dalam updateWorld
}

// Fungsi Penciptaan Mobil (Spawner)
void Intersection::spawnVehicles(float dt, double intervalNS, double intervalEW) {
    timerSpawn += dt;
    
    // Gunakan interval dinamis berdasarkan jam sibuk/sepi (rata-rata kecepatan spawn)
    double avgInterval = (intervalNS + intervalEW) / 2.0;
    ...
    if (timerSpawn > avgInterval) {
        ...
        if (r == 0 && countN < 6) vehicles.push_back(Vehicle({-4.0f, 0.5f, 45.0f}, {0.0f, 0.0f, -1.0f}, "N", c)); // Dari Utara
        else if (r == 1 && countE < 6) vehicles.push_back(Vehicle({45.0f, 0.5f, 4.0f}, {-1.0f, 0.0f, 0.0f}, "E", c)); // Dari Timur
        else if (r == 2 && countS < 6) vehicles.push_back(Vehicle({4.0f, 0.5f, -45.0f}, {0.0f, 0.0f, 1.0f}, "S", c)); // Dari Selatan
        else if (r == 3 && countW < 6) vehicles.push_back(Vehicle({-45.0f, 0.5f, -4.0f}, {1.0f, 0.0f, 0.0f}, "W", c)); // Dari Barat
    }
}

// Fungsi Pengatur Waktu Dunia (Berjalan Setiap Detik)
void Intersection::updateWorld(float dt, double intervalNS, double intervalEW) {
    timerLampu += dt;
    ...
    if (!isYellowPhase) {
        ...
        if (timerLampu > phaseDuration) {
            ...
            isYellowPhase = false; // Kuning Mati
            currentPhase = (currentPhase + 1) % 4; // Berputar 0(N) -> 1(E) -> 2(S) -> 3(W) -> Kembali 0(N)
        }
    }

    // Panggil Spawner mobil
    spawnVehicles(dt, intervalNS, intervalEW);

    // Di updateBehavior, kita meneruskan informasi apakah sedang lampu kuning atau tidak ke otak mobil
    for (auto& v : vehicles) {
        v.updateBehavior(dt, currentPhase, vehicles, isYellowPhase);
    }
...
// ==========================================
void Intersection::drawWorld3D() const {
    ...
    for(int i=0; i<4; i++) {
        ...
        if (currentPhase == i) {
            if (isYellowPhase) colorMid = YELLOW; // Menyala Kuning Tengah
...
// ==========================================
void Intersection::drawWorld2D(Camera camera) const {
    ...
    for(int i=0; i<4; i++) {
        ...
        if (screenPos.x >= 0 && screenPos.x <= GetScreenWidth() && screenPos.y >= 0 && screenPos.y <= GetScreenHeight()) {
            ...
            if (currentPhase == i) {
                ...
                int faseTunggu = (i - currentPhase + 4) % 4; 
